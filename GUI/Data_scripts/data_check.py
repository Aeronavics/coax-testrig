...
EXPECTED_LABEL = "Throttle (us)"            # This is the first expected label


EXPECTED_HEADER = ['Throttle (us)',	'Thrust (kg)', 'Top Motor Voltage (V)', 'Top Motor Current (A)', 'Bottom Motor Voltage (V)', 'Bottom Motor Current (A)', 'Top Motor Temperature (C)', 'Bottom Motor Temperature (C)']	
EXPECTED_ROW_SIZE = 7
...
def error_check(raw_data: list[Union[LF, list[str], str]], filename: str) -> list[Union[LF, list[str], str]]:
    ...
    for row in raw_data:
        add_row = True
        col_index = 0
        row.pop(-1)         # May cause problems if format ius fixed
        ...
        elif add_row == False and (row[0] != EXPECTED_LABEL) and row_index != 0:
...
def row_check(data: list[LF], filename: str) -> None:
    ...
    Args:
        ...
        filename (str): Name of current file
    """
    for index, row in enumerate(data[1:]): # Skips header rows
