...
def give_power_list(data: list[LF]) -> list[LF]:
    ...
    for row in data:
        top_motor_power = calc_power(row[TOP_V_INDEX], (row[TOP_I_INDEX] - CURRENT_OFFSET))
        bottom_motor_power = calc_power(row[BOTTOM_V_INDEX], (row[BOTTOM_I_INDEX] - CURRENT_OFFSET))
